...
def downloadFile(url:String, target_path:String):
    ...
    if response.status_code == 200:
        with open(target_path, 'wb') as f: # Somewhere here can be optimze the script to never download tar file, maybe...
            f.write(response.raw.read())


def extractTarFile(target_path:String):
    file = tarfile.open(target_path)
    file.extractall('./output') # creates folder labeled "output"
    file.close()
...
def main():
    # if you are trying to install a different model change the url and target_path
    ...
    target_path = 'CenterNet Resnet101 V1 FPN 512x512_coco17_tpu-8.tar.gz'
